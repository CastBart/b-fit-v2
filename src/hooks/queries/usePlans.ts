import { useMemo } from 'react'
import { useQuery } from '@tanstack/react-query'
import { getPlans } from '@/server/actions/plans'
import { offlineQueryFn } from '@/lib/react-query/offlineQueryFn'
import { filterPlans } from '@/lib/react-query/clientFilters'
import type { PlanFiltersInput } from '@/lib/validations/plan'

/**
 * Fetches all plans into a single canonical cache entry and applies
 * client-side filtering and pagination.
 */
export function usePlans(params: PlanFiltersInput = {}) {
  const allQuery = useQuery({
    queryKey: ['plans', 'all'],
    queryFn: offlineQueryFn(['plans', 'all'], async () => {
      const result = await getPlans({ page: 1, limit: 100 })
      if (!result.success || !result.data) {
        throw new Error(result.error || 'Failed to fetch plans')
      }
      return result.data
    }),
    networkMode: 'offlineFirst',
    staleTime: 1000 * 60 * 5,
    refetchOnWindowFocus: false,
    refetchOnReconnect: true,
  })

  const filtered = useMemo(() => {
    if (!allQuery.data) return undefined
    return filterPlans(allQuery.data, { page: 1, limit: 12, ...params })
  }, [allQuery.data, params])

  return { ...allQuery, data: filtered }
}
